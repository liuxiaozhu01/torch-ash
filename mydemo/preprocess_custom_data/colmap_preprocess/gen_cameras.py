...
if __name__ == '__main__':
    ...
    ## scale vertices by scale_mat_inv
    vertices = vertices @ np.linalg.inv(scale_mat).T
    ## convert vertices to non-homogeneous coordinates
    vertices = vertices[:, :3] / vertices[:, 3:]
    ## import trimesh
    trimesh.points.PointCloud(vertices).export(os.path.join(out_dir, 'scaled_points.ply'))
    ## ---------new added ----------
